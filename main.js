...
// Rayon orbital (px) par tier — container 420px
...
const QUESTS = {
  daily:[
    {id:'d_connect', name:'Connexion Quotidienne',desc:"Ouvrir le jeu",          icon:'🔐',reward:50, req:s=>true},
    {id:'d_click100',name:'Manivelle Active',      desc:'100 clics',              icon:'👆',reward:30, req:s=>s.totalClicks>=100},
    {id:'d_steam10k',name:'Production Journalière',desc:'10 000 vapeur',          icon:'💨',reward:80, req:s=>(s.dailyStats?.steamToday||0)>=10000},
  ...
  global:[
    ...
    {id:'g_rings25',name:'Seigneur Orbitaux', desc:'25 anneaux',            icon:'💍',reward:500,req:s=>Object.values(s.rings||{}).reduce((a,b)=>a+b,0)>=25},
  ],
  secret:[
    {id:'s_click10k',name:'Légende du Clic', desc:'10 000 clics',          icon:'🔥',reward:300,req:s=>s.totalClicks>=10000},
    {id:'s_boost200',name:'Démon de Vitesse',desc:'200% de boost',          icon:'⚡',reward:400,req:s=>s.maxBoostReached>=200},
  ],
};
const ACH_DEF = [
...
const state = {
  ...
  materials:0,gold:0,gears:{},artisans:{},rings:{},gearEvolutions:{},
  ...
  bulk:1,showTab:'gears',leftCollapsed:false,rightCollapsed:false,firstClick:false,
};
...
// Accès DOM sécurisé — ne crash jamais si un id est manquant
...
function load(){
  try{
    ...
    if(state.lastDailyReset!==today){state.lastDailyReset=today;state.dailyStats={steamToday:0,clicksToday:0};}
  }catch(e){console.warn('Load error:',e);}
}
function exportSave(){
  const b=new Blob([JSON.stringify(state,null,2)],{type:'application/json'});
  const a=document.createElement('a');
  a.href=URL.createObjectURL(b);a.download='steam_save.json';a.click();URL.revokeObjectURL(a.href);
}
function importSave(file){
  const r=new FileReader();
  r.onload=()=>{try{Object.assign(state,JSON.parse(r.result));save();fullRefresh();toast('✅ Import réussi');}catch(e){toast('❌ Import invalide');}};
  r.readAsText(file);
}
function reset(){
  ...
  GEAR_TYPES.forEach(g=>{state.gears[g.id]=0;state.rings[g.id]=0;state.gearEvolutions[g.id]={auto:true};});
  ARTISAN_TYPES.forEach(a=>{state.artisans[a.id]=0;});
  Object.assign(state,{steam:0,steamTotal:0,steamPerSecond:0,steamPerSecondBoost:0,materials:0,gold:0,totalEvolutions:0,totalClicks:0,clickBoost:0,maxBoostReached:0,lastClickTime:0,achievements:{unlocked:[],claimed:[]},recentAchievements:[],questsClaimed:[],questsCompleted:0,dailyStats:{steamToday:0,clicksToday:0},totalResets,firstClick:false});
  save();fullRefresh();toast('🔄 Réinitialisation effectuée');
}
function refundAll(){
  let gain=0;
  GEAR_TYPES.forEach(g=>{gain+=(state.gears[g.id]||0)*g.cost*.5;gain+=(state.rings[g.id]||0)*g.cost*.2;state.gears[g.id]=0;state.rings[g.id]=0;});
  ARTISAN_TYPES.forEach(a=>{gain+=(state.artisans[a.id]||0)*a.baseCost*.5;state.artisans[a.id]=0;});
  state.steam+=gain;state.steamTotal+=gain;recalc();save();fullRefresh();toast('💸 +'+fmt(gain)+' vapeur');
}

// ── MATHS ────────────────────────────────────────────────────
const artisanMult=id=>{const n=state.artisans[id]||0;const b=Math.floor(n/9),d=Math.floor(b/3),c=Math.floor(d/3);return Math.pow(3,b)*Math.pow(5,d)*Math.pow(10,c);};
...
function recalc(){state.steamPerSecond=GEAR_TYPES.reduce((s,g)=>s+gearTotal(g),0);state.steamPerSecondBoost=state.steamPerSecond*boostMult();}
const gearCostN=(gid,n)=>{const base=GEAR_TYPES.find(g=>g.id===gid);let c=0;for(let i=0;i<n;i++)c+=Math.ceil(base.cost*Math.pow(1.15,(state.gears[gid]||0)+i));return c;};
const maxBuyGear=gid=>{const base=GEAR_TYPES.find(g=>g.id===gid);return Math.max(0,Math.floor(Math.log(1+state.steam*.15/(base.cost*Math.pow(1.15,state.gears[gid]||0)))/Math.log(1.15)));};
...
function tick(){
  ...
  // Auto-évolution 10 gears → 1 ring
  GEAR_TYPES.forEach(g=>{
    if(!state.gearEvolutions[g.id]?.auto)return;
    const evo=Math.floor((state.gears[g.id]||0)/EVOLUTION_COST);
    if(evo>0){state.gears[g.id]-=evo*EVOLUTION_COST;state.rings[g.id]=(state.rings[g.id]||0)+evo;state.totalEvolutions+=evo;}
  });
  recalc();checkAchievements();
}
...
function renderShop(){
  ...
  list.innerHTML='';
  if(state.showTab==='gears')     GEAR_TYPES.forEach(g=>list.appendChild(buildGearItem(g)));
  else if(state.showTab==='artisans')  ARTISAN_TYPES.forEach(a=>list.appendChild(buildArtisanItem(a)));
  else if(state.showTab==='structures') ARTISAN_TYPES.forEach(a=>{if(state.artisans[a.id]>0)list.appendChild(buildStructItem(a));});
  else GOLD_RATES.forEach(r=>list.appendChild(buildGoldItem(r)));
...
function buildGearItem(g){
  ...
  const ringDots=rings>0?Array.from({length:Math.min(rings,6)},()=>`<div class="shop-prog-ring-dot" style="--gc:${g.color}"></div>`).join('')+(rings>6?`<span class="shop-prog-ring-extra">+${rings-6}</span>`:''): '';
  ...
  el.innerHTML=`
    ...
    <div class="shop-prog">
      <span class="shop-prog-label">${remainder}/${EVOLUTION_COST}</span>
      <div class="shop-prog-bar"><div class="shop-prog-fill" style="width:${pct}%;background:linear-gradient(90deg,${g.color}88,${g.color})"></div></div>
...
function buildArtisanItem(a){
  ...
  el.innerHTML=`
    ...
    <div class="meta">
      ...
      <div class="d">Mult: ×${fmt(artisanMult(a.id))} · Prod: ${fmt(a.production*n*artisanMult(a.id))}/s</div>
...
function buildStructItem(a){
  const n=state.artisans[a.id]||0,bro=Math.floor(n/9),dist=Math.floor(bro/3),cong=Math.floor(dist/3);
  const el=document.createElement('div');el.className='shop-item';
  el.innerHTML=`<div class="ico">${a.icon}</div><div class="meta"><div class="n">${a.name} T${a.id}</div><div class="d">×${fmt(artisanMult(a.id))}</div><div class="s">Confréries:${fmt(bro)} Quartiers:${fmt(dist)} Cong:${fmt(cong)}</div></div>`;
  return el;
}
function buildGoldItem(r){
  const canBuy=state.steam>=r.steamCost;
  const el=document.createElement('div');el.className='shop-item'+(canBuy?'':' locked');
  el.innerHTML=`<div class="ico">💰</div><div class="meta"><div class="n">${r.name}</div><div class="d">${fmt(r.steamCost)} 💨 → ${fmt(r.goldReward)} 💰</div></div><div class="item-footer"><button class="pill buy" data-gold="${r.id}" ${canBuy?'':'disabled'}>Échanger</button></div>`;
  return el;
}
...
function renderWorkbench(){
  ...
  let anyActive=false;
  GEAR_TYPES.forEach(g=>{
    const total=state.gears[g.id]||0,rings=state.rings[g.id]||0;
    if(total===0&&rings===0)return;
    anyActive=true;
    const remainder=total%EVOLUTION_COST;
    const slots=Array.from({length:EVOLUTION_COST},(_,i)=>`<div class="gr-slot${i<remainder?' filled':''}" style="--gc:${g.color}"></div>`).join('');
    const dots=rings>0?Array.from({length:Math.min(rings,8)},()=>`<div class="gr-ring-dot" style="--gc:${g.color}"></div>`).join('')+(rings>8?`<span class="gr-ring-extra">+${rings-8}</span>`:''): '';
    const row=document.createElement('div');row.className='gear-row';
    row.innerHTML=`<span class="gr-icon">${g.icon}</span><span class="gr-name">${g.name.split(' ').slice(1).join(' ')}</span><div class="gr-slots">${slots}</div><span class="gr-count">${remainder}/${EVOLUTION_COST}</span><div class="gr-rings">${dots}</div>`;
...
// ═══════════════════════════════════════════════════════════
// ORRERY — animation 60 fps, jamais appelé depuis renderShop
//
//  Zone 0  r < 75px      → mainGear (click)
//  Zone 1  r = tierR-18  → 10 slots par tier actif (⚙ / ○)
//  Zone 2  r = tierR     → 1 anneau par tier, épaisseur ∝ rings
//  Zone 3  badge ×N      → affiché en haut de l'anneau
// ═══════════════════════════════════════════════════════════
let gearRot=0;
function renderGearLayers(){
  const speed=1+state.clickBoost/100+state.steamPerSecond/500;
  gearRot=(gearRot+.5*speed)%360;
  const mainGearEl=$('mainGear');
  if(mainGearEl)mainGearEl.style.transform=`rotate(${gearRot}deg)`;

  const ringsEl=$('rings');
  const pyrEl  =$('pyramids');
  if(!ringsEl||!pyrEl)return;
  ringsEl.innerHTML='';
  pyrEl.innerHTML='';

  GEAR_TYPES.forEach(g=>{
    const tierIdx = g.id-1;
    const ringR   = TIER_RING_RADII[tierIdx];
    const slotR   = ringR-18;
    const rings   = state.rings[g.id]||0;
    const active  = (state.gears[g.id]||0)%EVOLUTION_COST;
  ...
  // Artisans en mode atelier
  const artEl=$('artisansLayer');
  if(artEl&&state.mode==='workshop'){
    ...
    ARTISAN_TYPES.forEach((a,i)=>{
      ...
      const d=document.createElement('div');d.className='artisan-group '+pos[i];
...
function renderStats(){
  ...
  set('steamCounter',      fmt(state.steam));
  set('steamRateTotal',    fmt(state.steamPerSecond));
  set('steamPerSecondBoost',fmt(state.steamPerSecondBoost));
  set('materialCount',     fmt(state.materials));
  set('goldCounter',       fmt(state.gold));
  set('steamTotalDisplay', fmt(state.steamTotal));
  set('gearCount',         fmt(totalGears()));
  set('artisanCount',      fmt(totalArtisans()));
  set('ringCountDisplay',  fmt(totalRings()));
  set('currentBoostDisplay','+'+Math.floor(state.clickBoost)+'%');
  set('goldTotalDisplay',  fmt(state.gold));
  const bf=$('boostFill');if(bf)bf.style.width=Math.min(100,state.clickBoost/3)+'%';
  const bp=$('boostPct');if(bp)bp.textContent=Math.floor(state.clickBoost)+'%';
  ...
  set('nextMilestone',fmt(target));
  const mp=$('milestoneProgress');if(mp)mp.style.width=Math.min(100,((state.steamTotal-prev)/(target-prev))*100)+'%';
  const rec=$('recentAchievements');
  if(rec)rec.innerHTML=state.recentAchievements.length
    ?state.recentAchievements.slice(0,5).map(a=>`<div class="recent-item"><span>${a.icon||'🏆'}</span><span>${a.name}</span><span style="color:var(--c-gold);margin-left:auto">+${a.reward}💰</span></div>`).join('')
...
function checkAchievements(){
  ACH_DEF.forEach(a=>{
    if(!state.achievements.unlocked.includes(a.id)&&a.req(state)){
      state.achievements.unlocked.push(a.id);state.achievements.claimed.push(a.id);
      state.gold+=a.reward;state.recentAchievements.unshift({...a});
      state.recentAchievements=state.recentAchievements.slice(0,6);
      toast(`🏆 ${a.name} · +${a.reward} 💰`);spawnCoins(window.innerWidth/2,window.innerHeight/2);
    }
  });
}
function renderAchievements(){
  ...
  g.innerHTML=ACH_DEF.map(a=>{
    const unl=state.achievements.unlocked.includes(a.id),clm=state.achievements.claimed.includes(a.id);
    return `<div class="ach-card ${clm?'claimed':''} ${!unl?'locked':''}"><div class="ach-icon">${a.icon}</div><div class="ach-name">${a.name}</div><div class="ach-desc">${a.desc}</div><span class="badge">${clm?'✅ Réclamé':unl?a.reward+' 💰':'🔒 Verrouillé'}</span></div>`;
  }).join('');
}
function renderQuests(type='daily'){
  ...
  g.innerHTML=(QUESTS[type]||[]).map(q=>{
    ...
    return `<div class="ach-card ${done?'claimed':''} ${claimed?'locked':''}"><div class="ach-icon">${q.icon}</div><div class="ach-name">${q.name}</div><div class="ach-desc">${q.desc}</div><div style="margin-top:8px"><span class="badge" style="display:inline-block;margin-bottom:6px">${done?'✅':'⏳'} ${done?'Complétée':'En cours'}</span><br><button class="pill buy" data-claim-q="${q.id}" data-qt="${type}" ${done&&!claimed?'':'disabled'}>${claimed?'Réclamé':'Réclamer +'+q.reward+' 💰'}</button></div></div>`;
  }).join('');
}
...
function floatText(x,y,t){
  ...
  d.style.cssText=`left:${x-20}px;top:${y-20}px`;document.body.appendChild(d);setTimeout(()=>d.remove(),870);
}
...
function fullRefresh(){recalc();renderStats();renderShop();renderWorkbench();renderGearLayers();}

// ── LOOP — animation uniquement, jamais renderShop ───────────
let frame=0;
function loop(){
  requestAnimationFrame(loop);
  tick();frame++;
  if(frame%2===0) renderStats();
  if(frame%3===0) renderGearLayers();
}
// Shop : mise à jour 1×/s indépendante de la loop
function shopTick(){renderShop();renderWorkbench();}

// ═══════════════════════════════════════════════════════════
// INIT — tout dans DOMContentLoaded pour garantir que le
// DOM est prêt avant d'attacher les listeners
// ═══════════════════════════════════════════════════════════
document.addEventListener('DOMContentLoaded',()=>{
  ...
  // ── Shop clicks (délégation sur le conteneur) ──────────
  $('shopList')?.addEventListener('click',e=>{
    ...
    if(btn.dataset.buy){
      ...
      if(state.steam>=cost&&(state.gears[a.gearCostType]||0)>=gNeed){
        ...
        toast(a.name+' ×'+n);recalc();renderShop();renderWorkbench();
      }
    }
    else if(btn.dataset.gold){
      const r=GOLD_RATES.find(x=>x.id===+btn.dataset.gold);
      if(r&&state.steam>=r.steamCost){
        state.steam-=r.steamCost;state.gold+=r.goldReward;
        toast('+'+fmt(r.goldReward)+' 💰');spawnCoins(window.innerWidth*.7,88);
        recalc();renderShop();
      }
    }
      ...
      if(evo>0){
        state.gears[gid]-=evo*EVOLUTION_COST;state.rings[gid]=(state.rings[gid]||0)+evo;state.totalEvolutions+=evo;
        toast('💍 +'+evo+' anneaux Tier '+gid);spawnCoins(window.innerWidth/2,window.innerHeight*.6);
        recalc();renderShop();renderWorkbench();
      }
    }
      ...
      if(q&&q.req(state)&&!state.questsClaimed.includes(q.id)){
        ...
        toast('📜 '+q.name+' +'+q.reward+' 💰');spawnCoins(window.innerWidth/2,window.innerHeight/2);
  ...
  $('leftToggle')?.addEventListener('click',()=>{
    ...
    const lt=$('leftToggle');if(lt)lt.textContent=state.leftCollapsed?'▶':'◀';
  });
  $('rightToggle')?.addEventListener('click',()=>{
    state.rightCollapsed=!state.rightCollapsed;
    $('rightPanel')?.classList.toggle('collapsed',state.rightCollapsed);
    const rt=$('rightToggle');if(rt)rt.textContent=state.rightCollapsed?'◀':'▶';
  });

  // ── Mode ──────────────────────────────────────────────
  $('etabliMode')?.addEventListener('click',  ()=>switchMode('etabli'));
  $('workshopMode')?.addEventListener('click',()=>{if(state.steamTotal>=10_000_000)switchMode('workshop');});

  // ── Modals ────────────────────────────────────────────
  $('achBtn')?.addEventListener('click',  ()=>{$('achievementModal')?.classList.remove('hidden');renderAchievements();});
  $('questBtn')?.addEventListener('click',()=>{$('questModal')?.classList.remove('hidden');renderQuests('daily');});
  $('closeAch')?.addEventListener('click',  ()=>$('achievementModal')?.classList.add('hidden'));
  $('closeQuest')?.addEventListener('click',()=>$('questModal')?.classList.add('hidden'));
  ...
  $('refreshBtn')?.addEventListener('click', ()=>{recalc();fullRefresh();});
  $('refundBtn')?.addEventListener('click',  refundAll);
  $('resetBtn')?.addEventListener('click',   reset);
  $('exportBtn')?.addEventListener('click',  exportSave);
  $('importBtn')?.addEventListener('click',  ()=>$('fileInput')?.click());
  $('fileInput')?.addEventListener('change', e=>{if(e.target.files[0])importSave(e.target.files[0]);});
