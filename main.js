...
const QUESTS = {
  ...
  global: [
    ...
    { id: 'g_art10', name: 'Équipe Complète',     desc: '10 artisans au total',         icon: '👥', reward: 100, req: s => Object.values(s.artisans || {}).reduce((a,b)=>a+b,0) >= 10 },
    { id: 'g_rings25',name:'Seigneur des Anneaux',desc: '25 anneaux productifs',        icon: '💍', reward: 500, req: s => Object.values(s.rings || {}).reduce((a,b)=>a+b,0) >= 25 },
  ],
  secret: [
...
const ACH_DEF = [
  ...
  { id: 'first_gear',  name: 'Premier Mécanisme',       desc: '1 engrenage fabriqué',  icon: '⚙️',  reward: 2,    req: s => (s.gears?.[1] || 0) >= 1 },
  ...
  { id: 'ring_5',      name: 'Seigneur des Anneaux',    desc: '5 anneaux',             icon: '💍',  reward: 100,  req: s => Object.values(s.rings || {}).reduce((a,b)=>a+b,0) >= 5 },
  { id: 'ring_25',     name: 'Souverain des Anneaux',   desc: '25 anneaux',            icon: '💎',  reward: 1000, req: s => Object.values(s.rings || {}).reduce((a,b)=>a+b,0) >= 25 },
  ...
  { id: 'reset_1',     name: 'Recommencement',          desc: 'Réinitialiser',         icon: '🔄',  reward: 5,    req: s => s.totalResets >= 1 },
];
...
const state = {
  ...
  replicas: 1, clickBoost: 0, maxBoostReached: 0, lastClickTime: 0,
...
function importSave(file) {
  ...
  r.onload = () => {
    try {
      ...
      save(); renderAll(); toast('✅ Import réussi');
...
function reset() {
  ...
  Object.assign(state, {
    ...
    clickBoost: 0, maxBoostReached: 0, lastClickTime: 0, replicas: 1,
  ...
  save(); renderAll(); toast('🔄 Réinitialisation effectuée');
}
...
const artisanMult = id => {
  ...
  const bro = Math.floor(n / 9);
  const dist= Math.floor(bro / 3);
  const cong= Math.floor(dist / 3);
...
const gearCostSingle = gid => Math.ceil(GEAR_TYPES.find(g=>g.id===gid).cost * Math.pow(1.15, state.gears[gid]||0));
...
const maxBuyGear = gid => {
  const base = GEAR_TYPES.find(g=>g.id===gid);
  return Math.max(0, Math.floor(Math.log(1 + state.steam * .15 / (base.cost * Math.pow(1.15, state.gears[gid]||0))) / Math.log(1.15)));
};
...
function buildGearItem(g) {
  ...
  el.innerHTML = `
    ...
    <div class="item-footer">
      ...
      <button class="pill" data-conv="${g.id}">Convertir (${Math.floor(n/EVOLUTION_COST)} paquets)</button>
    </div>`;
  return el;
}
...
function buildArtisanItem(a) {
  ...
  const cost = a.baseCost * bulk;
  const gNeed= a.gearCostAmount * bulk;
...
function renderGearLayers() {
  ...
  const artEl   = $('artisansLayer'); artEl.innerHTML = '';

  let rIdx = 0;
  GEAR_TYPES.forEach(g => {
    // Anneaux
    for (let i = 0; i < (state.rings[g.id] || 0); i++) {
      const d = document.createElement('div');
      d.className = 'ring';
      const sz = 220 + rIdx * 22;
      d.style.cssText = `width:${sz}px;height:${sz}px;border-color:${g.color};--spd:${8 + rIdx * 1.5}s`;
      ringsEl.appendChild(d); rIdx++;
    }
    // Mini engrenages pyramide
    const count = Math.min(state.gears[g.id] || 0, 16);
    for (let i = 0; i < count; i++) {
      const d = document.createElement('div');
      d.className = 'pyrGear';
      d.style.cssText = `--gc:${g.color};left:calc(50% + ${(i%4-1.5)*32}px - 11px);top:calc(50% + ${Math.floor(i/4)*32 - 50}px - 11px)`;
      pyrEl.appendChild(d);
    }
  });
  ...
  if (state.mode === 'workshop') {
    const positions = ['pos-1','pos-2','pos-3','pos-4','pos-5','pos-6','pos-7','pos-8'];
    ARTISAN_TYPES.forEach((a, i) => {
      if (!state.artisans[a.id]) return;
      const d = document.createElement('div');
      d.className = 'artisan-group ' + positions[i];
...
// ── RENDER ALL ─────────────────────────────────────────────────────────────
function renderAll() { recalc(); renderStats(); renderShop(); renderGearLayers(); }
...
$('shopList').addEventListener('click', e => {
  ...
  if (btn.dataset.art) {
    ...
    const n    = bulk || 1;
    const cost = a.baseCost * n, gNeed = a.gearCostAmount * n;
...
$('refreshBtn').onclick = () => { recalc(); renderAll(); };
